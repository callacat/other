...
async function fetchExternalHtml(url) {
    ...
    if (!response.ok) {
        throw new Error('Failed to fetch external HTML');
    }
...
export default {
    async fetch(request, env, ctx) {
        ...
        // 如果请求路径为根路径，则获取并返回外部HTML页面
        if (pathname === '/') {
            try {
                
                const htmlContent = await fetchExternalHtml(externalHtmlUrl);
                ...
                return new Response('Failed to fetch external HTML', {
                    status: 500,
                    headers: {
